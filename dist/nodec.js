...
function cleanup(noCleanup) {
    if (noCleanup) {
        return console.info('you opted for --noCleanup so all artifacts have been left in the following temp folder:', NodecFolders.root);
    }
...
function getDefaultTarget() {
    return `${os.type() === 'Linux' ? 'linux' : os.type() === 'Darwin' ? 'macos' : 'win'}-${os.arch() === 'arm64' ? 'arm64' : 'x64'}`;
}
/**
...
async function nodec() {
    ...
    const { entry, format, name, noCleanup, nodeFlags, nodeVersion, outDir, target } = await yargs
        ...
        .option('noCleanup', {
        default: false,
        description: 'if true, will leave all of the downloaded, bundled and compresses assets in a temporary folder on your machine, so you can inspect the state of them',
        type: 'boolean',
    })
        .option('nodeFlags', {
        default: [],
        description: 'one or more node.js flags to automatically set when executing your compiled application (like --experimental-require-module, --experimental-default-type and others). for a list of available flags, please refer to the official node.js documentation: https://nodejs.org/docs/latest/api',
        type: 'array',
    })
        .option('nodeVersion', {
        default: defaultNodeVersion,
        description: 'defines the version of NodeJS that will be used when compiling your standalone executable. Must be an explicit version. Semver ranges are not supported.',
        type: 'string',
    })
        .option('outDir', {
        default: process.cwd(),
        description: 'if set, uses this dir as the location where your binaries will be placed after compilation. Defaults to your CWD.',
        type: 'string',
    })
        .option('target', {
    ...
    try {
        if (!target.every(t => supportedOs.has(t))) {
            throw new Error(`you have provided one or more compilation targets that are not supported. ${Array.from(supportedOs).join(',')} are supported`);
        }
        await guardGoToolchain();
        const downloadLocations = await Promise.all(target.map(async (osname) => downloadNode(nodeVersion, osname)));
        const bundleLocations = await Promise.all(downloadLocations.map(async (nodePath) => bundleEntrypoint(entry, nodePath, nodeVersion, format)));
        const renderedLocations = await Promise.all(target.map(async (target, i) => compileBinary(bundleLocations[i], downloadLocations[i], nodeFlags.map(arg => String(arg)), target, name, outDir)));
