...
export async function guardGoToolchain() {
    ...
    if (!gobinpath) {
        throw new Error(`The Go toolchain is required to be installed, in order to compile your node application to a binary with nodec. To get started with Go, head to Go install guide:${os.EOL}https://go.dev/doc/install`);
    }
    ...
    if (!versionMatch.startsWith(minRequiredVersion) && sortedSemver[0] === versionMatch) {
        ...
        throw new Error(`The version of the Go toolchain yuo have installed is not supported. Wanted at least ${minRequiredVersion} but found ${versionMatch}`);
    }
}
